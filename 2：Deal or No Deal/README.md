...
选手 AI 保留一个箱子，逐个打开其他箱子；银行家 AI 选择来电时机、报价档位，并回应还价。你只负责观看并按回车控制节奏，终端会展示公开理由、报价与期望值的比较，以及最后的结算。

当前实现是 **Python 终端 + 两个 AI + 三个 LangGraph 节点**。使用 `TypedDict`、`StateGraph`、`add_messages`、`AIMessage.tool_calls`、`ToolMessage` 和条件边；一次 `invoke()` 运行一局，每次有效行动后等待回车。两个角色可以共用一个模型，也可以分别配置模型。
...
**每一次有效行动显示后都会暂停，按回车才执行下一次行动。** 选箱、每次开箱、银行家 `wait` / 报价、还价及回应都分别暂停，不会等到整轮开箱结束才停。第一次行动直接开始，最后一次行动直接显示结算；非法响应纠正和 HTTP 重试不算有效行动。暂停期间不会调用下一位 AI，人类不参与策略决策。Ctrl+C 可退出；输入关闭（EOF）会中止，不会自动跳过暂停。请使用可以输入回车的终端。
...
2. **execute**：校验候选响应、身份、阶段、参数和额度。合法才执行；成功后更新游戏事实、返回工具反馈并显示事件，若对局仍在进行则通过显示回调等待回车，再沿条件边继续；失败则反馈原因，让同一角色限次重试。
...
终端使用 Rich 排版：左侧是固定箱号，中间显示当前谈判和双方最近的公开理由，右侧是两列完整奖金表。已开出的奖金变灰、划线，并以 `x` 标记。报价区保留持箱均值与 `报价 / EV`，详细分析保存在报价事件中，可使用 `--record` 留作复盘。还价可以高于 EV。

终端达到 140 字符列时使用横向布局，较窄时改为纵向排列。逐步观战输出可回滚的静态快照，回车提示位于快照下方，等待输入时不刷新屏幕。输出重定向到文件或管道时使用简洁事件行，仍然等待标准输入中的回车。理由使用模型真实返回的 `reason`，主屏最多显示四行并标明截短，完整原文仍保存在事件中；等待状态不编造模型想法。`--quiet` 隐藏逐步局面，但仍显示回车提示和结算。更新后先运行 `python -m pip install -r requirements.txt` 安装显示依赖。
