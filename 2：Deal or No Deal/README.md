...
终端达到 140 字符列且高度足够时固定刷新；较窄时改为纵向排列，高度不足时输出可滚动的快照。输出重定向到文件或管道时使用简洁事件行。理由使用模型真实返回的 `reason`，主屏最多显示四行并标明截短，完整原文仍保存在事件中；等待状态不编造模型想法。`--quiet` 仍只显示结算。更新后先运行 `python -m pip install -r requirements.txt` 安装新增显示依赖。
